 ...
 * author : grp 1 
 * 
 * desc   : udp iterative server 
 *
 *          usage :  ./server_udp <port>
 *
 *          A basic flow for iterative udp server was as:
 *          create sock -> bind ->  recvfrom -> sendto -> close -> (recvfrm) 
 *
*************************************************************************/
...
int sendfile (int tx_sock,char* tx_file_name,int tran_byt_size,struct sockaddr* client,socklen_t client_addrlen) ;
...
int main (int argc, char* argv[]) {
	...
	printf("-----------Multiprocessor connectionless(udp) server------------------\n");
	...
	// set reuse option for socket to avoid debugging resource exhausting
	int opt = 1;
	if (setsockopt(server_sock, SOL_SOCKET, SO_REUSEADDR | SO_REUSEPORT,
				&opt, sizeof(opt))) {
		printf ("Fail to create sock opt\n");
		exit(-1);
	}
	...
	while (cnt<10) {
		...
		*/
		...
		if ( child==0 ) { // child process 
			printf ("Created chld process:%d.\n",(int)getpid());
			// Receive message from clienBindt ;
			//int rcd = 0;
			// TCP
			/*
			if ( PROTOCOL_TYP==0) {
				rx = read (accept_sock,&tx_file_name,MAX_FILE_NAME_LEN);//sizeof(rcd));
			...
			*/
			...
			int tx = sendfile (server_sock,tx_file_name,MAX_TX_SIZE,(struct sockaddr*) &client,client_addrlen);
			if (tx<0){
			...
			close (server_sock); //child process finish
			exit(0);
		}
